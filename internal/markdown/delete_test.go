...
func TestDeleteArea(t *testing.T) {
	...
	t.Run("deleting a document hides it from reads and keeps its history", func(t *testing.T) {
	...
	t.Run("only the tombstone version is marked deleted", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("writing to a deleted path resurrects it and continues the numbering", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("an ordinary conflict does not claim the document was deleted", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("deleting twice is not found rather than a second tombstone", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("deleting with a stale version is a conflict", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("every problem with one delete is reported in one pass", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("a deletion is announced", func(t *testing.T) {
	...
	// written" case is what TestDeleteArea's "a deletion is announced" case
	// cannot reach, and Task 3's correction 4 says so for Write: moving the
	// publish inside withTx's callback leaves the announcement test green,
	// because every failure a refusal produces never reaches a publish
	// wherever the call sits. This one fails
	...
	t.Run("no deletion is announced when the tombstone cannot be written", func(t *testing.T) {
		svc, _, hub, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// refuse.
	t.Run("the guarded update is what refuses a delete that raced an edit", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("the actor of a deletion is recorded", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("a tombstones body is still readable as a version", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
...
// records the same gap for Write: a publish sitting as the last statement
// *inside* withTx's callback differs from the correct placement only by the
// commit that follows, and every other failure a delete can produce returns
// from the callback before that statement runs. Verified: with the publish
