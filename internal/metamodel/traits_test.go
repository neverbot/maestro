...
func TestTraitsArea(t *testing.T) {
	t.Parallel()
	...
	// TestTraitsArea's "symmetric cannot gate" case pins the first coherence
	// rule: an edge that means the same in both directions cannot also gate in
	// one.
	t.Run("symmetric cannot gate", func(t *testing.T) {
		_, svc, project := traitsFixture(t)
	...
	// TestTraitsArea's "annotation cannot share a type with anything" case
	// pins the second rule, and pins it against **every** other word in the
	// vocabulary rather than against one: `annotation`'s conflict list is
	// derived from AnalysisTraits, and a derivation that quietly stopped
	// covering a seventh trait is the shape this repository keeps producing.
	t.Run("annotation cannot share a type with anything", func(t *testing.T) {
		_, svc, project := traitsFixture(t)
	...
	// TestTraitsArea's "a type cannot gate in both directions" case pins the
	// third rule.
	t.Run("a type cannot gate in both directions", func(t *testing.T) {
		_, svc, project := traitsFixture(t)
	...
	// repeated trait cannot mean anything a caller intended, and folding it
	// silently would make what comes back differ from what was sent.
	t.Run("a trait declared twice is refused as a typo", func(t *testing.T) {
		_, svc, project := traitsFixture(t)
	...
	// case separates the two states the whole column exists to carry: NULL is
	...
	// TestTraitsArea's "the trait vocabulary is one set in go and in the
	// database" case is the bidirectional guard the whole pairing rests on,
	// and it is bidirectional on purpose: a one-way test passes against a
