...
func TestRepairArea(t *testing.T) {
	t.Parallel()
	a := newArea(t)
	...
	// TestRepairArea's "a repair refuses a set key the type does not declare"
	// case keeps a caller's own mistake from being reported two hundred times
	// as a property of the game's content.
	t.Run("a repair refuses a set key the type does not declare", func(t *testing.T) {
		f := newRepairFixture(t, 3)
		ctx := context.Background()
	...
	// TestRepairArea's "repairing an undeclared type is not found" case: the
	// address is a key, like every other read on this surface, and a key that
	// names nothing is not_found rather than a pass over zero rows.
	t.Run("repairing an undeclared type is not found", func(t *testing.T) {
