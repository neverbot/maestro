...
func TestDiffArea(t *testing.T) {
	t.Parallel()
	a := newArea(t)

	t.Run("a diff shows only what changed", func(t *testing.T) {
	...
	// case pins the hunk header arithmetic in the one case where the two sides
	// differ in length: a header that counted the wrong side renders as a
	// broken patch in every tool that reads unified diffs.
	t.Run("a diff of an added and a removed line counts both sides", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// rules" case and TestDiffArea's "a deleted documents diff numbers the
	// empty side by GNU rules" case pin hunkRange against GNU diff -U3's own
	// output for the one case that discriminates it from printing the count
	// verbatim: a completely empty side. `diff
	...
	t.Run("an added documents diff numbers the empty side by GNU rules", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// one that does" case pins splitLines' trailing-newline rule: without it
	// the two bodies differ by a phantom empty line and every such pair reads
	// as a two-line change.
	t.Run("a document that does not end in a newline diffs against one that does", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// as changed" case pins that the coarse fallback does not carry the
	// up-to-three lines of context the fine path's trim backs off. Those lines
	// are genuinely identical on both sides; printing them under both '-' and
	// '+' is arithmetically consistent with a header that counts what it
	// prints, but it is not a change and a coarse hunk does not need context
	// of its own.
	t.Run("a coarse diff does not reprint its common head and tail as changed", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestDiffArea's "a huge but local change is still computed line by line"
	// case is the other half of the bound: the table is built over what
	// survives the common prefix and suffix, so a one-line edit in the middle
	// of a very long document is fine and only a genuinely large *difference*
	// falls back.
	t.Run("a huge but local change is still computed line by line", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("diffing another games document is not found at the path", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
	...
	// case pins the defect the prose sub-project's end-to-end run found, and
	// it is the same class of defect that run exists for: a state change that
