...
func TestSearchArea(t *testing.T) {
	...
	t.Run("a document is found by a word in its body", func(t *testing.T) {
	...
	// read-back this plan's header asks of every feature: a value the search
	// path writes and no caller can read is a column that could be dropped
	// from the select list with nothing going red, which is the write-only
	...
	// name, because what has to hold is that no such field exists — the shape
	// TestListArea's "a listing row carries no body at all" case and
	// TestVersionsArea's "a history row carries no body at all" case both
	// take, and the same two checks: a substring match on the lowered field
	// name, so a field spelled any way that reads as a body or a frontmatter
	...
	t.Run("a document the query names outranks one that only mentions it", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("a search hit names the entities the document is attached to", func(t *testing.T) {
		svc, entities, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		newQuest(t, entities, game, "wanted-hogger", "Wanted: Hogger")
	...
	t.Run("a deleted document is not searchable", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		if _, err := svc.Write(ctx, game, markdown.WriteInput{
	...
	// TestSearchArea's "a search kind is bounded as the callers own argument"
	// case keeps the kind filter from reaching Postgres unbounded, the hole
	// Task 7's correction 3 and Task 8's correction 1 each found one step
	// further along: an invalid UTF-8 byte reaches the server as a byte
	// sequence it refuses outright (SQLSTATE 22021), which would land on the
	// default arm as internal_error over a value the caller itself supplied.
	t.Run("a search kind is bounded as the callers own argument", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestSearchArea's "a search limit defaults and is honoured" case pins
	// that a limit applies at all — sixty matching documents against a default
	...
	// TestSearchArea's "a word past the index bound is stored but not
	// findable" case is what keeps markdown.MaxIndexedChars honest. The
	// constant mirrors a literal inside 0007_documents.sql's generated
	// expression and nothing in Postgres reads it, so without this the search
