...
func TestSchemaVocabArea(t *testing.T) {
	...
	// compares metamodel.FieldTypes against the constants declared in
	...
	// TestSchemaVocabArea's "every listed field type is accepted by validate"
	// case carries the list one step along, which is where a vocabulary
	// usually dies: correct in its own declaration and unknown to the code
	...
	// TestSchemaVocabArea's "the semantic role vocabulary is one set in go and
	// in the database" case is the same bidirectional pairing
