...
func TestAuthorsArea(t *testing.T) {
	t.Parallel()
	a := newArea(t)
	...
	// TestAuthorsArea's "an author that is gone is no author rather than an
	// empty one" case pins the case the zero Author is for: both audit columns
	// are ON DELETE SET NULL, so a user who is really deleted leaves the row
	// naming nobody, and the answer is no kind and no id rather than a kind
	// naming nothing.
	t.Run("an author that is gone is no author rather than an empty one", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestAuthorsArea's "a conflict names who wrote the version to merge onto"
	// case is the third of the three findings, end to end through Write: a
	// caller told to merge onto version 2 is told who wrote version 2 and
	// when, which is what decides whether it merges or asks.
	t.Run("a conflict names who wrote the version to merge onto", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
