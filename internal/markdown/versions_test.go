...
func TestVersionsArea(t *testing.T) {
	t.Parallel()
	a := newArea(t)

	t.Run("history is newest first and carries no bodies", func(t *testing.T) {
	...
	// assertion the test above cannot make from a field it can name: what has
	// to be pinned is the
	...
	// turns this red.
	t.Run("a history row carries no body at all", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("history pages with a cursor belonging to its own document", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// nothing there.
	t.Run("a cursor from another games history is refused", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
	...
	t.Run("a malformed history cursor is invalid input at its own path", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// case does not pin the clamp policy itself -- with only two versions in
	// the fixture, clamping to MaxHistoryPage, folding onto DefaultHistoryPage
	// and applying no bound at all all return the same two rows, so no
	// assertion here can tell them apart. That policy is paging.Size's and is
	// pinned directly, in internal/paging/cursor_test.go's
	// TestSizeClampsRatherThanFoldingOntoTheDefault. What this test does pin:
	// History does not error or truncate below a real page's worth of rows
	// when asked for more than the cap.
	t.Run("a history page asks for too much and gets the cap", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		writeVersions(t, svc, game, "bible", 2)
	...
	t.Run("read version returns the body as it stood", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		writeVersions(t, svc, game, "bible", 3)
	...
	t.Run("reading another games version is not found", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		azeroth := newGame(t, pool, "azeroth")
		outland := newGame(t, pool, "outland")
	...
	// TestVersionsArea's "two games sharing one path keep separate histories"
	// case is the isolation the test above cannot show, because a miss looks
	// the same whatever the reason: with the path taken in both games, each
	// history must answer with its own rows and its own project id.
	t.Run("two games sharing one path keep separate histories", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		azeroth := newGame(t, pool, "azeroth")
		outland := newGame(t, pool, "outland")
	...
	// TestVersionsArea's "a version row cannot claim a game its document does
	// not belong to" case is the database-level half of the same invariant,
	// and it is here rather than only in internal/db because this is the
	// package that writes the column: InsertDocumentVersion sets project_id
	// from the caller's own resolved id, and 0007_documents.sql's composite
	// key is the only thing that would refuse a mismatch.
	t.Run("a version row cannot claim a game its document does not belong to", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		azeroth := newGame(t, pool, "azeroth")
		outland := newGame(t, pool, "outland")
	...
	t.Run("revert writes forward rather than rewriting history", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		writeVersions(t, svc, game, "bible", 3)
	...
	t.Run("revert keeps the stored title rather than deriving it again", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// RevertInput's own comment: kind is a property of the document and is not
	// versioned, so restoring prose must not move the shelf the document sits
	// on.
	t.Run("reverting leaves the documents kind alone", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// leaves the document alive" case pins Revert's own comment. The
	// alternative — re-deleting — would mean one call doing two things, with
	// the delete's own expected_version never checked.
	t.Run("reverting to a tombstone restores its body and leaves the document alive", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		writeVersions(t, svc, game, "bible", 1)
	...
	// pins the case Revert's own doc comment names as following from writeWith
	// rather than being chosen here: the document itself is currently deleted
	// (not merely the version being restored), and a revert brings it back
	// live, the same way a Write to a deleted path does. Delete at version 4
	// (tombstone lands at 5), then revert to version 1 -- the document comes
	// back live at version 6, its content equal to version 1's, deleted_at
	// cleared.
	t.Run("reverting a deleted document resurrects it", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("reverting to a missing version names the argument", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		writeVersions(t, svc, game, "bible", 2)
	...
	t.Run("reverting a document that was never there is not found", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("revert is announced with the version it restored", func(t *testing.T) {
	...
	// case is what the test above cannot reach, and it is here because the
	// plan predicted the announcement test would stay green against a publish
	// moved inside withTx — it does, since every refusal returns before the
	// publish wherever the call sits. This one asserts the other direction: a
	// revert whose to_version does not exist announces nothing at all, so a
	// subscriber never hears about a restore that did not happen.
	t.Run("no revert is announced when the revert is refused", func(t *testing.T) {
		svc, _, hub, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		writeVersions(t, svc, game, "bible", 2)
	...
	// TestVersionsArea's "a version is addressed by its own document" case is
	// what the cross-game tests cannot show: with two documents inside *one*
	// game, the project filter separates nothing and only the document filter
	// on the two version queries answers the right rows. Dropping either
	// query's document_id filter is what turns this red.
	t.Run("a version is addressed by its own document", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestVersionsArea's "a revert records who made it" case pins that the
	// audit columns of the version a revert writes name the reverter, not the
	// author of the version whose content was restored: the restore is a new
	// edit and someone made it.
	t.Run("a revert records who made it", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
...
// TestVersionsArea's "no revert is announced when the revert is refused"
// case cannot reach, and this package has now found the same gap three
// times — Task 3's correction 4 for Write, Task 4's correction 4 for
// Delete, and here. A publish sitting as the last statement *inside*
// withTx's callback differs from the correct placement only by the commit
// that follows, and every other failure a revert can produce returns from
// the callback before that statement runs: verified, with the publish moved
