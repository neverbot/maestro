...
func TestMoveArea(t *testing.T) {
	...
	// TestMoveArea's "a moved document keeps its history and its numbering"
	// case is the whole reason Move exists. Before it, the only way to fix a
	// path was to write the document at the new one and delete the old one,
	// which forked the history: the new path began at version 1 holding none
	// of what came before. So this asserts the opposite of that fork, in the
	// terms it would have shown up in — one document, one id, continuous
	// numbering, every past version still readable, and the old path free
	// afterwards.
	t.Run("a moved document keeps its history and its numbering", func(t *testing.T) {
	...
	// the half of the move that lives in the record rather than in the row. A
	// history that carried only today's path would show three snapshots of
	// zones/duskwood/lore, two of which were written while the document was
	// called something else, with nothing to say so.
	t.Run("a history shows where each version was written", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// old content" case pins what makes that history row legible as a move
	// rather than as an edit: its content equals its predecessor's and its
	// path does not.
	t.Run("the version a move appends carries the new path and the old content", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// links keying on document_id rather than on a path: nothing about the
	// attachments has to be maintained by Move, so this test is what proves
	// that claim rather than repeating it.
	t.Run("a moved document keeps its links", func(t *testing.T) {
	...
	// case-folding decision. documents_path_key folds case, so the two
	// spellings are one address; this repository refuses a case-only
	// respelling of a row key and of a document path on a write, and a move is
	// refused for the same reason.
	t.Run("a case only move is refused as a respelling", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// destination" case: a move never merges two documents, and the refusal
	// has to name the end that is in the way rather than reporting a bare
	// conflict.
	t.Run("moving onto a live document is refused at the destination", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestMoveArea's "moving onto a deleted path says its history is still
	// there" case: the unique index covers soft-deleted rows, so a deleted
	// path is still taken. A caller told only "that path exists" would go
	// looking for a document no read answers for.
	t.Run("moving onto a deleted path says its history is still there", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// case. The recovery is a write, and the refusal has to say so: a bare
	// not_found would read as "no such document" for one whose whole history
	// is still on file.
	t.Run("moving a deleted document says to bring it back first", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestMoveArea's "moving with a stale version is a conflict" case pins the
	// guard on MoveDocument. A move advances the version, so an unguarded one
	// would land on top of an edit the caller never read.
	t.Run("moving with a stale version is a conflict", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestMoveArea's "every problem with one move is reported at its own end"
	// case. Two bad paths in one call are two field errors under two names, so
	// a caller can tell which end it typo'd — the discrimination
	// pathProblemsAt exists for.
	t.Run("every problem with one move is reported at its own end", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestMoveArea's "a move is announced with both ends" case. A move is the
	// one change in this domain a subscriber cannot discover by re-reading
	// what it holds: the path it knows answers not_found, indistinguishably
	// from a deletion. So the payload carries both ends.
	t.Run("a move is announced with both ends", func(t *testing.T) {
	...
	// publish runs after the transaction, so a refused move must announce
	// nothing at all — the case TestMoveArea's "a move is announced with both
	// ends" case cannot reach on its own.
	t.Run("no move is announced when the move is refused", func(t *testing.T) {
		svc, _, hub, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestMoveArea's "two opposite moves do not deadlock" case drives the lock
	// ordering in lockBothEnds. Two transactions moving a to b and b to a
	// would, if each locked its own source first, hold the lock the other
	...
	// callers" case. writeWith records row.Path and not in.Path, because a
	// write addressed under another casing reaches the existing document and
	// UpsertDocument leaves the path column alone — so the caller's spelling
	// is not the version's address, and recording it would put a spelling in
	// the history that no reader of the document ever sees.
	t.Run("a version records the stored spelling and not the callers", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		...
		// (TestDeleteArea's "a path is matched without regard to case on delete"
		// case) and appends the tombstone, so it is the write path in this
		// package that can reach InsertDocumentVersion under a spelling the row
		// does not carry.
		if _, err := svc.Delete(ctx, game, markdown.DeleteInput{
