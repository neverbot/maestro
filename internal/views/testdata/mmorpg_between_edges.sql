...
node_rows (id, key, name, type_key, set_name, role, source_id, target_id, fields, rank, depth, attrs, ambiguous) AS (
    ...
    FROM (
        ...
        FROM (
    SELECT e.id, e.key, e.name, et.key, s0.set_name, $7::text,
           NULL::uuid, NULL::uuid, NULL::jsonb, $8::integer, s0.depth, jsonb_strip_nulls(jsonb_build_object($4::text, to_jsonb(e.name), $5::text, e.fields -> $6)), false
    FROM s0
...
edge_rows (id, key, name, type_key, set_name, role, source_id, target_id, fields, rank, depth, attrs, ambiguous) AS (
    ...
    SELECT r.id, NULL::text, NULL::text, rt.key, NULL::text, NULL::text,
           r.source_id, r.target_id, NULL::jsonb, $10::integer, NULL::integer, NULL::jsonb, NULL::boolean
    FROM relations r
