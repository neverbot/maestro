...
func TestDocumentsArea(t *testing.T) {
	t.Parallel()
	a := newArea(t)

	t.Run("writing a document creates it at version one and it reads back byte for byte", func(t *testing.T) {
	...
	t.Run("the actor of a write reads back on the document", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("the first write also writes version one with its author and message", func(t *testing.T) {
		...
		// row cannot claim a game its document does not belong to" case inserts a
		// row naming another game's document and watches the composite key refuse
		// it, and TestVersionsArea's "two games sharing one path keep separate
		// histories" case pins that this listing filters on it.
	...
	t.Run("a missing expected version is invalid input rather than a guess", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("expecting a version of a document that does not exist is not found", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("a path is matched without regard to case on read", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("two concurrent writes leave one winner and no gap in the versions", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("writing a path another game uses creates a second document", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		azeroth := newGame(t, pool, "azeroth")
		outland := newGame(t, pool, "outland")
	...
	t.Run("a document event reaches a viewer and a token alike", func(t *testing.T) {
		svc, _, hub, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("a kind and a message are bounded as the callers own arguments", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// pins review finding 1 on Task 3: `kind` is a property of the document,
	// not of any one edit, so a caller that does not mention it must not erase
	// it. A nil Kind preserves whatever is stored; a Kind pointing at "" is a
	// caller explicitly clearing the label, and that must go through too.
	t.Run("an edit that omits kind leaves it unchanged", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
...
// cannot reach: a publish sitting as the last statement *inside* withTx's
// callback, which differs from the correct placement only by the commit
// that follows. Proved necessary by mutation — moving the publish inside
...
// newUser inserts a designer. The address the caller passes is what the
// test reads, so it is kept and made unique rather than replaced: email
// is a unique column and a database now holds many claims' users.
