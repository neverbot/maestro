...
func TestKindsArea(t *testing.T) {
	t.Parallel()
	a := newArea(t)

	// TestKindsArea's "the kind catalogue lists what the game uses" case is
	// the discovery half of the free-text kind. Both filters compare kinds and
	// nothing told a caller which kinds exist; this is that answer, in the
	// shape the game summary already uses for entity types.
	t.Run("the kind catalogue lists what the game uses", func(t *testing.T) {
	...
	// TestKindsArea's "an empty games kind catalogue is an empty list and not
	// nil" case. A game with no prose is the first state anyone sees, and a
	// client that has to tell [] from null before it can loop has been handed
	// two spellings of one fact — the rule DocumentPage and the game summary
	// both follow.
	t.Run("an empty games kind catalogue is an empty list and not nil", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		game := newGame(t, pool, "azeroth")
	...
	// is the folding decision. Both filters match kinds case-insensitively, so
	// "Lore" and "lore" select one set of documents and must be one row
	// counting all of them. Two rows would each carry a count belonging to
	// neither filter result.
	t.Run("two spellings of one kind are one catalogue row", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// kind kept alive by nothing but a tombstone is a filter value whose
	// answer, under the default listing and under search, is an empty page —
	// so the catalogue must not offer it. And because deletion is soft, the
	// kind has to come back when the document does.
	t.Run("a deleted documents kind leaves the catalogue", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// is one grouped scan and the project filter is the only thing keeping it
	// inside one game.
	t.Run("a kind catalogue is per game", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
	...
	// counts" case is the property that makes this safe on a page load and in
	// an agent's context, and it is asserted the way the game summary's own
	// test asserts it: seed many rows under few kinds and check that no row's
	// path appears in the answer.
	t.Run("the kind catalogue does not grow with the documents it counts", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
