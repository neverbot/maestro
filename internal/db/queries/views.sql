...
--
--   * On views, every one of them is. A key is caller-supplied and names no
--     parent, and a view id is a value a previous answer handed back, so
--     nothing but the project filter keeps either inside one game. That
--     includes GetViewByID: a view id in a caller's hand is not authority
--     to read it. TestViewsArea's "reading another games view is not found"
--     case pins both, by key and by id, with a positive control in the same
--     test.
--   * On view_refs, the filter is defence in depth *for this package's own
--     callers* and is load-bearing for anyone else's, which is a narrower
--     claim than the one this comment first made and is the honest one.
--     0008_views.sql gives the table a composite FOREIGN KEY (view_id,
--     project_id) REFERENCES views (id, project_id), so no ref row can hold
--     a view id under one project id and a different project id of its own;
--     and UpsertView, the only caller that writes here, has just written
--     the view row in the same transaction. So dropping the filter changes
--     no answer any service call can produce. It does not follow that
--     nothing can observe it: the composite key constrains what a row may
--     *hold*, not which rows a DELETE may *match*, so DeleteViewRefs called
--     with a foreign view id and this game's project id would clear another
--     game's index. TestViewsArea's "the view queries addressing a row by
--     id are scoped to the project" case drives these statements directly,
--     because that is the only way to observe a filter every service path
--     has already made redundant — a defence-in-depth claim is worth what
--     the test behind it is worth. InsertViewRef writes project_id as a
--     column value, which is what makes the composite key check anything at
--     all.
--   * On ListViewsBrokenByType, load-bearing again, and for its own caller
--     rather than for a hypothetical one: it is handed a bare type id. Its
--     two project filters mask each other and only their joint removal is
--     observable; the statement's own comment says so and names the test
--     that asks the question across the games.
--   * On view_positions, load-bearing on the read and on both deletes, for
--     the same reason as view_refs and with the same caveat: a view id is a
--     value a previous answer handed back, and the composite foreign key
--     says what a row may hold rather than which rows a read or a DELETE
--     may match. UpsertViewPosition is the interesting one and it needs
--     *two* mechanisms: the composite keys refuse a parent from another
--     game on the insert path, and they check nothing at all on the ON
--     CONFLICT path, where the stored row keeps its own project id -- so
--     the DO UPDATE carries a project guard of its own. That is written up
--     on the statement, because it was a real cross-game overwrite before
--     it was a comment. Each statement here says which of its filters is
--     doing work and which is redundant with the one above it.
--
...
-- that is a decision rather than an omission.** A background is written by
-- views.set_background (the views plan, Task 14) and belongs to the picture
-- rather than to the query; listing them here would make every ordinary
-- edit of a query or a renderer parameter silently detach the world map a
-- designer put behind it, because a caller that says nothing about a
-- background would be saying "none". The same distinction markdown's
-- WriteInput.Links draws with a pointer, drawn here by the column simply
-- not being writable from this path. TestViewsArea's "an ordinary upsert
-- leaves a background standing" case writes a background directly and edits
-- the view through this statement, which is what makes the decision above
...
-- Matched without regard to case, through views_key_key (project_id,
-- lower(key)), so a caller that reads with one spelling and writes with
-- another meets one row rather than two.
SELECT * FROM views
...
-- held, so the spelling and the version a caller is told about are the ones
-- its own write will actually meet. Without the lock the read runs against
-- the transaction's snapshot, and a caller racing an in-flight edit is told
-- to merge onto a version that is already stale by the time it retries.
...
-- would have met" case pins exactly that.
SELECT * FROM views
WHERE project_id = sqlc.arg('project_id')::uuid AND lower(key) = lower(sqlc.arg('key')::text)
FOR UPDATE;
...
-- views.ViewsDependingOn takes a bare type id and resolves nothing inside
-- the game first, so the isolation this statement enforces is the only
-- isolation its own caller has. TestViewsArea's "views depending on a type
-- are found by id with their pointers" case asks the other game about this
-- game's type id, on both arms, with the positive control in the same test
-- — the discrimination in the rest of that test comes from the two games'
-- type ids differing, which passes either way.
...
-- coordinates.
--
-- So the DO UPDATE is guarded on the stored row's own project id, and the
-- statement is execrows rather than exec: a guard that matches nothing
-- updates nothing and reports zero, which Go turns into a refusal
-- (positions.go) instead of the silent success a bare guard would give.
-- TestPositionsArea's "positions of another game are not reachable" case
-- drives both paths -- an entity with no stored position for the 23503, one
-- with a stored position for the guard -- and reads the coordinates back.
...
-- A view id is a value a previous answer handed back, so nothing but that
-- filter keeps this read inside one game; dropping it hands another game's
-- positions to a caller holding a leaked view id, which TestPositionsArea's
-- "positions of another game are not reachable" case asks for directly. The
-- two join conditions -- e.project_id = p.project_id and et.project_id =
-- e.project_id -- are implied by 0008_views.sql's and 0004_metamodel.sql's
-- composite foreign keys and by the primary keys they join on, so each is
-- redundant with the filter above it and deleting either alone changes no
-- answer. They are written because a join to a project-scoped table that
-- does not carry the scope is the shape this repository has lost a filter
-- to six times, not because a test can tell them apart.
...
-- assets.go is where that is decided and TestAssetsArea's "the mime is
-- sniffed not trusted" case is what pins it. The table's CHECK on mime is a
-- closed list that does not include SVG, and is the backstop for a write
-- path that does not come through assets.go.
--
...
-- names no parent, so nothing else scopes this read; without it a caller
-- lists every game's images. TestAssetsArea's "assets of another game are
-- not listed" case asks for it directly, with a positive control in the
-- same test.
--
...
-- is a value a previous answer handed back and names no parent, so a leaked
...
-- it is 0008_views.sql's ON DELETE SET NULL job, and doing it here as well
-- would make the constraint unobservable through this package -- the
-- placement of that action is exactly the kind of thing only a constraint
-- pins, and a Go statement that quietly did the same work would leave a
-- dropped SET NULL green. TestAssetsArea's "deleting an asset nulls the
-- background of every view using it" case asserts the null the constraint
-- writes and the two defaults this statement writes, which is why the
-- division of labour is legible from the test.
--
...
-- another game cannot become this views background" case drives both,
