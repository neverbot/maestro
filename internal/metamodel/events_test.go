...
func TestEventsArea(t *testing.T) {
	t.Parallel()
	a := newArea(t)
	...
	// TestEventsArea's "no event is published for a refused write" case covers
	// the refusals that never reach the database and the ones that reach it
	// and are rolled back. An announcement is an instruction to re-read;
	// announcing a change that did not happen makes a client re-read for
	// nothing at best, and at worst teaches it that the value it already had
	// is new.
	t.Run("no event is published for a refused write", func(t *testing.T) {
		pool := a.pool
		hub := realtime.NewHub()
		svc := metamodel.New(pool, hub)
		ctx := context.Background()
	...
	// TestEventsArea's "pruned endpoint lists are published in sort order not
	// database order" case closes the locking verification's finding 2:
	// RemoveEntityType's `sort.Slice(pruned, …)` over `pruned[i].Key <
	// pruned[j].Key` is defensive against RETURNING order the database does
	// not promise, but nothing pinned it — the whole suite, including the test
	// correction 23 wrote for it, passes with the sort deleted, because
