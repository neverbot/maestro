...
// gated is the gated fixture in a project of the area's own database.
func (a area) gated(t *testing.T) game {
	t.Helper()
	return gatedIn(t, a.game(t))
}
...
func TestReach(t *testing.T) {
	...
	t.Run("a symmetric edge is followed both ways", func(t *testing.T) {
		g := a.gated(t)
		g.entity(t, "quest", "p")
		g.entity(t, "quest", "q")
	...
	t.Run("an annotation type is not followed at all", func(t *testing.T) {
		g := a.game(t)
		g.declareEntityType(t, "quest")
	...
	t.Run("under all a both gated entity needs both gates reachable", func(t *testing.T) {
		g := a.gated(t)
	...
	t.Run("the all fixpoint terminates on a cyclic graph and says why", func(t *testing.T) {
		g := a.gated(t)
	...
	t.Run("an invalid edge still gates unless the caller excludes it", func(t *testing.T) {
		g := a.gated(t)
		g.entity(t, "quest", "a")
		g.entity(t, "quest", "b")
	...
	t.Run("a seed key that does not resolve is not found and not an empty seed set", func(t *testing.T) {
		g := a.gated(t)
	...
	t.Run("a seed key from another game is not found", func(t *testing.T) {
		mine := a.gated(t)
	...
	t.Run("an empty seed set with ungated off is refused", func(t *testing.T) {
		g := a.gated(t)
		g.entity(t, "quest", "a")
	...
	// game's entities.
	t.Run("a seed route contributes its steps as seeds", func(t *testing.T) {
		g := a.gated(t)
	...
	t.Run("a seed route from another game is not found", func(t *testing.T) {
		mine := a.gated(t)
		mine.entity(t, "quest", "mine")
		theirs := mine.sibling(t)
		theirs.declareEntityType(t, "quest")
	...
	t.Run("a reach walk cannot leave its project through a rogue edge", func(t *testing.T) {
	...
	t.Run("a depth above the cap is refused rather than clamped", func(t *testing.T) {
		g := a.gated(t)
		g.entity(t, "quest", "a")
		_, err := g.analysis.Reach(context.Background(), Params{
	...
	t.Run("a depth limited walk says so", func(t *testing.T) {
		g := a.gated(t)
		for _, key := range []string{"a", "b", "c", "d"} {
			g.entity(t, "quest", key)
		}
...
func containmentGame(t *testing.T, a area) game {
	t.Helper()
	g := a.game(t)
