...
func resolveInto(cat *Catalogue, q *Query, st *staleness) (*Resolved, []metamodel.FieldError) {
	...
	for i := range q.Traverse {
		...
		// A step with no to_type reaches entities of any type, so a predicate on
		// it can only name built-ins: no schema is common to every type it could
		// land on. fieldScope says exactly that instead of blaming a type the
		// caller never named — TestResolveArea's "a step without a to type admits
		// only builtins" case pins the wording.
