...
func TestSearchArea(t *testing.T) {
	...
	t.Run("search finds by name and by every text shape a field can hold", func(t *testing.T) {
		pool := a.pool
	...
	// TestSearchArea's "search matches across types and narrows to one" case
	// pins the decision that a search is over a game rather than over a type:
	// an agent looking for a name does not know which type the game modelled
	// it as.
	t.Run("search matches across types and narrows to one", func(t *testing.T) {
		pool := a.pool
		svc := metamodel.New(pool, nil)
		ctx := context.Background()
		project := newProject(t, pool)
	...
	// TestSearchArea's "search names a mistyped type key" case pins that a
	// narrowing filter behaves like a listing's: a mistyped key is named, not
	// answered with nothing.
	t.Run("search names a mistyped type key", func(t *testing.T) {
		pool := a.pool
		svc := metamodel.New(pool, nil)
		ctx := context.Background()
		project := newProject(t, pool)
	...
	// TestSearchArea's "search ranks the name match first" case pins the
	...
	// TestSearchArea's "search still finds a word only a field carries" case
	// pins that weighting the name did not turn search into a name lookup: a
	// word that appears nowhere but in a field is still found, and still found
	// alone.
	t.Run("search still finds a word only a field carries", func(t *testing.T) {
		pool := a.pool
		svc := metamodel.New(pool, nil)
		ctx := context.Background()
		project := newProject(t, pool)
	...
	// TestSearchArea's "search is scoped to its game" case pins the project
	// filter, which is the only thing keeping a query inside one game: the
	// query text names no parent whose composite key could scope it.
	t.Run("search is scoped to its game", func(t *testing.T) {
		pool := a.pool
		svc := metamodel.New(pool, nil)
		ctx := context.Background()
	...
	// TestSearchArea's "search answers an over large limit with more than the
	// default" case pins the distinction relationPageSize's doc comment argues
	// for, at the one place a smaller test cannot see it: with fewer rows than
	...
	// TestSearchArea's "only the indexed head of a long field is searchable"
	...
	// another.
	t.Run("a search cursor belongs to its query", func(t *testing.T) {
		pool := a.pool
		svc := metamodel.New(pool, nil)
		ctx := context.Background()
		project := newProject(t, pool)
	...
	t.Run("a search cursor with a forged position is malformed", func(t *testing.T) {
		pool := a.pool
		svc := metamodel.New(pool, nil)
		ctx := context.Background()
		project := newProject(t, pool)
