...
func TestRenderersArea(t *testing.T) {
	t.Parallel()
	a := newArea(t)

	t.Run("the catalogue is closed and names itself back", func(t *testing.T) {
		g, _ := a.games(t)
	...
	// TestRenderersArea's "a renderer parameter it does not declare is
	// refused" case is the reason renderer_params is not a free jsonb blob in
	// practice: a typo'd rank_dircetion that is stored, returned and silently
	// ignored forever is the most expensive shape of bug this sub-project has,
	// and the refusal has to name what *was* accepted or an agent cannot find
	// its typo.
	t.Run("a renderer parameter it does not declare is refused", func(t *testing.T) {
		g, _ := a.games(t)
	...
	// TestRenderersArea's "nested refuses a query with no containment edges"
	// case carries its positive control in the same test: a refusal that
	// always fired would pass without one, and the control is a query that
	// *does* draw the relation contain_via names.
	t.Run("nested refuses a query with no containment edges", func(t *testing.T) {
		g, _ := a.games(t)
	...
	t.Run("timeline refuses an axis field that is not number or ordered enum", func(t *testing.T) {
		g, _ := a.games(t)
	...
	// TestRenderersArea's "an axis declared two ways is two axes" case is the
	// failure one step along from the type check: number and enum are each
	// admitted, so a scope holding one of each passes "is it number or enum"
	// while having no single axis to draw. The fixture declares min_level as a
	// number on quest and as an enum on region, and rank as the same three
	// options in two different orders — which is a set a predicate compares
	// happily and a sequence an axis is drawn from.
	t.Run("an axis declared two ways is two axes", func(t *testing.T) {
		g, _ := a.games(t)
		both := resolveFor(t, g,
	...
	// TestRenderersArea's "a timeline span has both ends on one axis" case
	// pins the Requires closure that compares the two ends of a span: a number
	// start with an enum end is two axes, and one of them would be placed
	// arbitrarily.
	...
	// TestRenderersArea's "a field key parameter names a field the saved query
	...
	// TestRenderersArea's "an axis some type in scope does not declare is
	// refused" case is requireDeclaredAs's own doc comment, which for one
	// round said the code refused this and did not. Declaring the key as a
	// different type on a second type is the rare mistake; drawing two types
	// and remembering the fields of one of them is the common one, and it
	// makes exactly the picture the comment describes — the quests placed, the
	// zones vanished, and what is left looking right.
	t.Run("an axis some type in scope does not declare is refused", func(t *testing.T) {
		g, _ := a.games(t)
	...
	// TestRenderersArea's "every bad column comes back with its index" case is
	// three assertions the list shape needs and no scalar parameter does:
	// every bad column is reported, each at its own index, and the *code* of
	// the answer does not depend on the order the columns were written in. The
	// last is the one that matters most — two wrong columns returning
	// query_invalid or renderer_requirements depending on which was typed
	// first is this package's shape-wins rule breaking at the one place a list
	// could break it.
	t.Run("every bad column comes back with its index", func(t *testing.T) {
		g, _ := a.games(t)
	...
	// TestRenderersArea's "a background is not a renderer parameter" case pins
	// the correction above from the other side: the three names are refused as
	// unknown parameters, so an agent that read a stale table and sent one is
	// told, rather than having a background silently stored where nothing
	// reads it.
	t.Run("a background is not a renderer parameter", func(t *testing.T) {
		g, _ := a.games(t)
		quests := resolveFor(t, g, carryingQuery)
	...
	// TestRenderersArea's "map asks for the coordinates its mode reads" case
	// pins the two halves of the mode switch: a coordinate field is required
	// in fields mode and read by nothing in manual mode, and a parameter read
	// by nothing is this sub-project's most-repeated defect.
	t.Run("map asks for the coordinates its mode reads", func(t *testing.T) {
		g, _ := a.games(t)
		quests := resolveFor(t, g, carryingQuery)
	...
	// TestRenderersArea's "layered needs the edges it ranks by" case is the
	// requirement addressed at the renderer rather than at a parameter: no one
	...
	// TestRenderersArea's "graph refuses edge labels no edge carries" case
	// pins the one boolean in this catalogue that asks the query for
	// something: edge_labels over a query that labels nothing draws nothing
	// and says nothing.
	...
	// TestRenderersArea's "a slot parameter names a slot the query declares"
	// case pins the split this catalogue makes for a channel: a renderer names
	// a *slot*, because the query already decides what a slot reads —
	// including the one hop to a neighbour, which no renderer can take for
	// itself.
	t.Run("a slot parameter names a slot the query declares", func(t *testing.T) {
		g, _ := a.games(t)
	...
	// TestRenderersArea's "a table column is something the envelope carries"
	// case is the contract in its purest form: a renderer consumes the
	// envelope, so a column naming a field the run will not carry is a blank
	// column in a saved view.
	...
	// TestRenderersArea's "a shape problem is answered before a requirement"
	// case pins the ordering of the two codes. They cannot travel in one error
	// — a QueryError carries one code — and answering the requirement first
	// would tell an agent its query is wrong when what is wrong is its typing.
	t.Run("a shape problem is answered before a requirement", func(t *testing.T) {
		g, _ := a.games(t)
	...
	// TestRenderersArea's "every problem of one class comes back at once" case
	// is this package's standing rule applied here: an agent fixing three
	// parameters should learn about three, not discover them one call apart.
	...
	// TestRenderersArea's "a renderer is judged against a resolved query" case
	// refuses the nil that would otherwise make every requirement in this file
	...
	// TestRenderersArea's "a table does not refuse a query that draws edges"
	// case pins a deliberate non-refusal, which is worth a test for the same
	// reason a refusal is: the next reader will ask why the "reads nothing"
	// rule that refuses map's x_field does not refuse a table's edges. Because
	// one envelope serves every renderer, and swapping graph for table must
	// never require rewriting the query.
	t.Run("a table does not refuse a query that draws edges", func(t *testing.T) {
		g, _ := a.games(t)
		checkPasses(t, CheckRenderer(RendererTable, map[string]any{"columns": []any{"@name"}},
			resolveFor(t, g, chainQuery)))
	})

	// TestRenderersArea's "a renderer is judged against a whole resolved
	// query" case is the nil check one step along: a *Resolved is exported, a
	// Go caller can build one by hand, and a missing Query or Catalogue panics
	...
	// stored" case is the assertion behind this file's first rule.
	// renderer_params is a free jsonb blob on the wire; a parameter declared
	// here and judged by nothing would be stored, returned and ignored
	// forever, which is the failure this whole catalogue exists to prevent.
	// Every parameter of every renderer is driven through a value its kind
	// cannot accept, and the refusal has to name that parameter.
	t.Run("every declared parameter is checked not just stored", func(t *testing.T) {
		g, _ := a.games(t)
		r := resolveFor(t, g, chainQuery)
	...
	// TestRenderersArea's "a parameter kind cannot be half added" case is the
	// reason the kinds are a table rather than a switch, stated as an
	// assertion, and it is bidirectional for the reason the operator table's
	// own guard is: a kind used by a parameter and missing from the checkers
	// is a parameter production refuses to judge, and a checker no parameter
	// uses is a kind somebody meant to declare.
	t.Run("a parameter kind cannot be half added", func(t *testing.T) {
	...
	// TestRenderersArea's "a parameter name carries one kind across the
	// catalogue" case is the guard over the one thing the per-renderer tables
	// cannot see. `group_by` is a projection slot on graph and on table; if
	// one of them were changed to text, the description would print two kinds
	// under one prose line, and a misspelt slot name on a table would be
	...
	// TestRenderersArea's "an enum parameter declares its values and nothing
	// else does" case keeps Values and kindEnum in step in both directions:
	// values on a parameter of another kind are read by nothing, and an enum
	// with none admits nothing at all.
	...
	// TestRenderersArea's "every required parameter is refused when missing"
	// case drives the Required flag itself, which is otherwise a boolean read
	// by one line nobody exercises.
	...
	// description is generated from them" case is the guard over the prose an
	// agent reads. Task 15's tool description is built from this catalogue, so
	// the two must not be able to disagree: a renderer added with no
	// parameters ships an undocumented knob, a parameter left out of the
	...
	// TestRenderersArea's "an untyped step switches off the field checks" case
	// records, as an assertion, the permission Task 9 wrote up: a traverse
	// step with no to_type reaches entities of any type, so no schema applies
	// and no field key can be judged — for a renderer parameter exactly as for
	// a projection. The control is the same query with the step typed, where
	// the same value is refused.
	t.Run("an untyped step switches off the field checks", func(t *testing.T) {
		g, _ := a.games(t)
	...
	// TestRenderersArea's "an optionless enum is no axis even if the schema
	// column holds one" case closes the gap between "refused at upsert" and
	// "true". metamodel.Schema.Validate refuses an enum with no options, which
	// made "an enum axis is ordered by its options" unreachable through the
	// API rather than a rule the axis held — nothing revalidates a schema on
	// the way *out* of the database, so a row written straight into the schema
	// column loads and is believed. The catalogue mutated here is exactly what
	// such a load produces.
	t.Run("an optionless enum is no axis even if the schema column holds one", func(t *testing.T) {
		g, _ := a.games(t)
	...
	// TestRenderersArea's "every type naming parameter kind is a recorded
	// reference" case is the both-arms guard for typeNamingKinds, which two
	// things have to agree on: the refs a save writes, and the resolution a
...
// that TestRenderersArea's "every declared parameter is checked not just
// stored" case can drive every declared parameter through the checker its
// kind names.
