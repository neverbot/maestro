...
// seedDocuments writes n documents under a prefix, so a listing has enough
// rows to page. Sixty, when the default page is fifty: a fixture smaller
// than the default cannot tell "the default applies" from "no bound
// applies", and one smaller than the page a test asks for cannot tell a
// clamp from a fold onto the default. See TestListArea's "the limit is
// clamped rather than folded onto the default" case, which is the reason
...
func TestListArea(t *testing.T) {
	...
	// TestListArea's "a listing is summaries in path order and every field
	// reads back" case reads every field of DocumentSummary back through List,
	// which is this domain's public reader for a listing. The rule this plan's
	// header states — a value written by one path and read by none is how a
	// relation's fields stayed write-only through nine review rounds — applies
	// to a projection as much as to a column: a field of this type that no
	// test reads is a field that can be dropped from the select list without
	// anything going red.
	t.Run("a listing is summaries in path order and every field reads back", func(t *testing.T) {
	...
	// TestListArea's "a listing row carries no body at all" case pins the
	// absence over every field of the type rather than over a field it can
	// name, because what has to hold is that no such field exists — the same
	// shape TestVersionsArea's "a history row carries no body at all" case
	// takes for a version row, and the same two checks: a substring match on
	// the lowered field name, so a field spelled any way that reads as a body
	// or a frontmatter survives a rename, and a scan of every field's rendered
	...
	t.Run("a listing pages and its cursor belongs to its filter", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// elsewhere" case is the entity half of the fingerprint, which the
	// neighbouring test cannot reach: an entity filter needs an entity, and
	// the cursor has to come *from* the filtered listing to prove the part is
	// in the digest at all.
	t.Run("a cursor from an entity filtered listing is refused elsewhere", func(t *testing.T) {
	...
	// the shape a game with no prose answers with: [] and not null, the same
	// decision TestLinksArea's "a document with no attachments is an ordinary
	// document" case makes for links, and asserted through encoding/json for
	// the same reason — non-nilness is not the claim, what a client receives
	// is.
	t.Run("an empty listing is an empty page and not nil", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		game := newGame(t, pool, "azeroth")
	...
	// filtered listing" case is why the entity filter is an EXISTS and not a
	// join: with a join, a document attached to two entities comes back twice
	// on a listing filtered by one of them, and the page's row count stops
	// being a count of documents. A caller paging that listing would see the
	// same path twice and its page of fifty would hold fewer than fifty
	// documents.
	t.Run("a document attached to two entities appears once in a filtered listing", func(t *testing.T) {
		svc, entities, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		newQuest(t, entities, game, "wanted-hogger", "Wanted: Hogger")
	...
	// TestListArea's "an entity filter is bounded before postgres sees it"
	// case is Task 7's correction 3 applied to this listing: an entity key
	// holding an invalid UTF-8 byte reaches Postgres as a byte sequence it
	// refuses outright (SQLSTATE 22021), which would land on the default arm
	// as internal_error over a value the caller itself supplied.
	t.Run("an entity filter is bounded before postgres sees it", func(t *testing.T) {
		svc, entities, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestListArea's "a path prefix and a kind are bounded as the callers own
	// arguments" case covers the other two filter strings, which reach the
	// query as text and would otherwise reach Postgres unbounded for the same
	// 22021.
	t.Run("a path prefix and a kind are bounded as the callers own arguments", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// TestListArea's "every problem with one listing is reported in one pass"
	// case keeps this call to the rule Write, Delete and LinkRemove already
	// follow: a caller whose prefix and whose kind are both wrong hears about
	// both, instead of fixing one, calling again and learning about the other.
	t.Run("every problem with one listing is reported in one pass", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		game := newGame(t, pool, "azeroth")
	...
	t.Run("naming only one half of the entity filter is invalid input", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("deleted documents are absent unless asked for", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	t.Run("a listing never crosses games", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		azeroth := newGame(t, pool, "azeroth")
		outland := newGame(t, pool, "outland")
	...
	// default" case needs a fixture larger than the default page, or clamping
	// to the cap and folding onto the default return the same rows and the
	// test proves nothing: sixty documents against a default of fifty is what
	// separates them. The cap itself is not reachable from here — asking for
	// the cap and asking for no bound at all both return the sixty rows that
	// exist — and that policy is pinned against paging.Size directly, in
	// internal/paging/cursor_test.go's
	// TestSizeClampsRatherThanFoldingOntoTheDefault.
	t.Run("the limit is clamped rather than folded onto the default", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
		seedDocuments(t, svc, game, "lore", 60)
	...
	// case is what TestListArea's "the limit is clamped rather than folded
	// onto the default" case's sixty-row fixture cannot prove: with sixty rows
	// against a cap of two hundred, "clamped at the cap" and "no bound at all"
	// answer identically, since there is nothing past the cap to lose. This
	// fixture has more rows than MaxDocumentPage, so the two only agree if the
	// cap is genuinely applied.
	t.Run("a cap exceeding fixture is actually capped at the cap", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		ctx := context.Background()
		game := newGame(t, pool, "azeroth")
	...
	// keeps this listing's malformed-cursor answer the one every listing in
	// Maestro gives: the caller's own argument, at path cursor, with the
	// recovery in the message — not an internal_error over a value the caller
	// supplied.
	t.Run("a listings cursor is refused when it is not one", func(t *testing.T) {
		svc, _, _, pool := a.service(t)
		game := newGame(t, pool, "azeroth")
